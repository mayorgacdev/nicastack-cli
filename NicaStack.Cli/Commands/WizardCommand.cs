    ...
    public override int Execute(CommandContext context, Settings settings)
    {
        ...
        var selection = AnsiConsole.Prompt(
            new SelectionPrompt<TemplateDefinition>()
                .Title("[bold green]Selecciona el template que quieres generar[/]")
                .UseConverter(template => $"{template.Name} [grey]({template.Id})[/]")
                .AddChoices(templates));

        var projectName = AnsiConsole.Ask<string>("[bold white]Nombre del proyecto/modulo:[/]");
        var gitInit = AnsiConsole.Confirm("Inicializar git al finalizar?", defaultValue: true);
